...
class TransformerAgent(nn.Module):
    def __init__(self, vocab_size, actions, max_seq_len, embedding_dimension=32, n_attention_head=4, layers=2, nplayers=3,  dim_feed_forward=256, device='cpu'):
        ...
        self.stack = nn.TransformerEncoder(
            encoder_layer=encoder_layer, num_layers=layers)
        self.policy = nn.Linear(
            in_features=embedding_dimension, out_features=actions)
        self.value = nn.Linear(
            in_features=embedding_dimension, out_features=nplayers)
    ...
    def forward(self, obs, legal_mask=None):
        assert str(
            obs.device) == self.device, f"obs={obs.device}, self={self.device}"
        batch_size, seq_len = obs.shape
        x = self.embedding(obs)
        positions = torch.arange(seq_len, device=self.device).unsqueeze(
        ...
        value = self.value(x)
        # we are storing a value for each player and we are making them sum to 1
        value = torch.softmax(value, dim=-1)
        return policy, value

    def save(self, file_name):
        data = dict(vocab_size=self.embedding.num_embeddings, actions=self.policy.out_features, max_seq_len=self.pos_embedding.num_embeddings, embedding_dimension=self.embedding.embedding_dim,
                    n_attention_head=self.stack.layers[0].self_attn.num_heads, layers=len(self.stack.layers), nplayers=self.nplayers, dim_feed_forward=self.stack.layers[0].linear1.out_features, state=self.state_dict())
    ...
    def step(self, state: Dict) -> int:
        ...
        with self.training_mode(False):
            with torch.no_grad():
                obs = torch.tensor(state['obs'], dtype=torch.long).unsqueeze(
                    0).to(self.device)
                legal_actions = torch.tensor(
                    list(legal_actions.keys()), dtype=torch.long, device=self.device)
                logits, _ = self(obs)
                mask = torch.ones(
                    self.actions, dtype=torch.bool, device=self.device)
    ...
    def load(cls, file_name, device="cpu", train=True):
        ...
        # Get nplayers from saved data, default to 3 for backward compatibility
        nplayers = data.get('nplayers', 3)
        agent = cls(vocab_size=data['vocab_size'], actions=data['actions'], max_seq_len=data['max_seq_len'], embedding_dimension=data['embedding_dimension'],
                    n_attention_head=data['n_attention_head'], layers=data['layers'], nplayers=nplayers, dim_feed_forward=data['dim_feed_forward'], device=device)
