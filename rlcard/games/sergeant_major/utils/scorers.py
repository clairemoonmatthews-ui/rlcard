'''
    File name: sergeant_major/scorers.py
...
if TYPE_CHECKING:
    from ..game import SergeantMajorGame
...
def get_payoff_sergeant_major_v0(player: SergeantMajorPlayer, game: 'SergeantMajorGame') -> int:
    ...
    if player.player_id == 0:
        ...
        if not isinstance(score_player_move, ScoreNorthMove):
            raise SergeantMajorProgramError("score_player_move must be ScoreNorthMove.")
    else:
        score_player_move = moves[-1]
        if not isinstance(score_player_move, ScoreSouthMove):
            raise SergeantMajorProgramError("score_player_move must be ScoreSouthMove.")
    deadwood_count = score_player_move.deadwood_count
    return deadwood_count


def get_payoff_sergeant_major_v1(player: SergeantMajorPlayer, game: 'SergeantMajorGame') -> float:
