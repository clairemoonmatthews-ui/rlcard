...
from .round import SergeantMajorRound
from .judge import SergeantMajorJudge
...
class SergeantMajorGame:
    ...
    def __init__(self, allow_step_back=False):
        '''Initialize the class SergeantMajorGame
        '''
        self.allow_step_back = allow_step_back
        self.np_random = np.random.RandomState()
        self.judge = SergeantMajorJudge(game=self)
        self.settings = Settings()
        self.actions = None  # type: List[ActionEvent] or None # must reset in init_game
        self.round = None  # round: SergeantMajorRound or None, must reset in init_game
        self.num_players = 3

    def init_game(self):
        ''' Initialize all characters in the game and start round 1
        '''
        dealer_id = self.np_random.choice([0, 1, 2])
        # if self.settings.dealer_for_round == DealerForRound.North:
        #     dealer_id = 0
        # elif self.settings.dealer_for_round == DealerForRound.South:
        #     dealer_id = 1
        self.actions = []
        self.round = SergeantMajorRound(dealer_id=dealer_id, np_random=self.np_random)
        for i in range(3):
            num = 16 
