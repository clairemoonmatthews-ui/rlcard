'''
    File name: sergeant_major/judge.py
...
from .utils.scorers import SergeantMajorScorer
...
class SergeantMajorJudge:

    '''
        Judge decides legal actions for current player
    '''

    def __init__(self, game: 'SergeantMajorGame'):
        ''' Initialize the class SergeantMajorJudge
        :param game: SergeantMajorGame
        '''
        self.game = game
        self.scorer = SergeantMajorScorer()
...
def get_going_out_cards(hand: List[Card], going_out_deadwood_count: int) -> Tuple[List[Card], List[Card]]:
    ...
    '''
    if not len(hand) == 11:
        raise SergeantMajorProgramError("len(hand) is {}: should be 11.".format(len(hand)))
