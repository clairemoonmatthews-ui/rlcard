'''
    File name: sergeant_major/round.py
    Author: Claire Matthews
    Date created: 10/9/2025
'''
from typing import TYPE_CHECKING
if TYPE_CHECKING:
    from .utils.move import SergeantMajorMove

from typing import List

from rlcard.games.sergeant_major.dealer import SergeantMajorDealer
...
from .utils.sergeant_major_error import SergeantMajorProgramError

from .player import SergeantMajorPlayer
from . import judge

from rlcard.games.sergeant_major.utils import melding
from rlcard.games.sergeant_major.utils import utils
...
class SergeantMajorRound:
    ...
    def __init__(self, dealer_id: int, np_random):
        ...
        self.dealer = SergeantMajorDealer(self.np_random)
        self.players = [SergeantMajorPlayer(player_id=0, np_random=self.np_random), SergeantMajorPlayer(player_id=1, np_random=self.np_random)]
        ...
        self.move_sheet = []  # type: List[SergeantMajorMove]
        player_dealing = SergeantMajorPlayer(player_id=dealer_id, np_random=self.np_random)
    ...
    def get_current_player(self) -> SergeantMajorPlayer or None:
    ...
    def draw_card(self, action: DrawCardAction):
        ...
        if not len(current_player.hand) == 10:
            raise SergeantMajorProgramError("len(current_player.hand) is {}: should be 10.".format(len(current_player.hand)))
    ...
    def discard(self, action: DiscardAction):
        ...
        if not len(current_player.hand) == 11:
            raise SergeantMajorProgramError("len(current_player.hand) is {}: should be 11.".format(len(current_player.hand)))
    ...
    def score_player_0(self, action: ScoreNorthPlayerAction):
        ...
        if not self.current_player_id == 0:
            raise SergeantMajorProgramError("current_player_id is {}: should be 0.".format(self.current_player_id))
    ...
    def score_player_1(self, action: ScoreSouthPlayerAction):
        ...
        if not self.current_player_id == 1:
            raise SergeantMajorProgramError("current_player_id is {}: should be 1.".format(self.current_player_id))
        current_player = self.get_current_player()
        best_meld_clusters = melding.get_best_meld_clusters(hand=current_player.hand)
        best_meld_cluster = [] if not best_meld_clusters else best_meld_clusters[0]
        deadwood_count = utils.get_deadwood_count(hand=current_player.hand, meld_cluster=best_meld_cluster)
